...
cell abi_show_results(cell thiscommand, cell toCubeN, cell toFaceN, cell level)
{
  ...
  for (counter=0;counter<4; counter++)
  {
    ...
    {
      abi_CMD_NET_TX(0, 2, data);
      abi_CMD_NET_TX(1, 2, data);
      abi_CMD_NET_TX(2, 2, data);
    }
...
cell zoneType[][] = { {BLUE_WALL | TURN_180_DEGREE, PILL, BLUE_CORNER | TURN_270_DEGREE},
...
new levelMap []{} = [{0,2,1, 4,5,3, 2,8,1, 2,4,5, 1,8,3, 0,5,4, 0,1,2, 4,3,5},
...
Parse (number, xCenterPos, yCenterPos, angle, axis = 0, reverse = 0){
    new power = 1;
    ...
    do {
        ...
        abi_CMD_BITMAP (RESOURCES_NUMBERS + number % 10, xDigitPos, yDigitPos, angle, 0);
...
GetDirectionTile (&directionTileX, &directionTileY, direction) {
    switch (direction) {
...
GetIndexBeforeRotation (angle, &x, &y) {
    new i = x, j = y;
    switch (angle) {
        break;case 3: {
            x = j;
...
GetDiagonalNeighbour (&myFace, cube, face) {
...
CheckGameOver () {
    if (pacmanDead) {
        ...
        SetGameState (FSM:death);
    }

    if (pillsCollected == maximumPillsOnLevel) {
        isLevelFinished = 2;
        SetGameState (FSM:results);
    }
}
    ...
    if (who == 0) {
        ...
        if (actorView & 0x02) {
            ...
            if (!wasIEaten) {
                ...
                abi_CMD_BITMAP (GHOST_PICS + (actorsSupportData[who] & 0x0F) + ghostAnim{actorAnimFrame}, actorPosX, actorPosY, angle, 0);
...
ONTICK () {
    ...
    if (!isTimerPaused && ((gameState == FSM:countdown) || (gameState == FSM:death))) {
        ++ticksTillStart;
    }
